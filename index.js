...
function hand(event){
    ...
    let DoctorName = event.target[0].value
    let DoctorId = parseFloat(event.target[1].value)
    let Specialization = event.target[2].value
    let Experience = (event.target[3].value)
    let Email = event.target[4].value
    let MobileNo = parseFloat(event.target[5].value)


  if (!(DoctorName||DoctorId||Specialization||Experience||Email||MobileNo)){
 ...
 function showdata(data){
    tbody.innerText = ""
     data.forEach(function(ele , i){
        ...
        td2.innerText = ele.DoctorName

        let td3 = document.createElement("td")
        td3.innerText = ele.DoctorId

        let td4 = document.createElement("td")
        td4.innerText = ele.Specialization
